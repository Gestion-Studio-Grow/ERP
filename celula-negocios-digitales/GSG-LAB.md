...
> y de venta. Su salida es un **producto para la mesa de dirección** (el Panel de Dirección, ADR-028).
>
...
- **El producto para la dirección:** el **Panel de Dirección** (`panel/panel.html`), publicado en la app
  como `/operador/direccion` (plano de plataforma, ADR-028).
...
> (ADR-028), reporta con los estados canónicos (🟢/✅/🔒) y deja handoff en `docs/PROXIMOS-PASOS.md`.
...
- **Decisiones estructurales** (ej. publicar el panel como producto) → `/sesion-arquitectura` + ADR
  (ya: **ADR-028**, plano de plataforma).
...
- **Persistencia:** este doc (roster) + `MOTOR-SPRINT-CICLICO.md` (método) + `ADR-CELULA-001` (aprendizajes)
  + `ADR-028` (el producto en la app). Nada del lab vive solo en el chat.
...
docs/adr/ADR-028-*.md             el Panel de Dirección como producto del control-plane
