...
function renderCarrito(): void {
  ...
  if (resumen.vacio) {
    cont.innerHTML = `<p class="soft">Tu carrito está vacío. <a href="${ruta("/")}#catalogo">Elegí una plantilla →</a></p>`;
    return;
  }
  ...
  cont.innerHTML = `
    ...
    <div class="cart-total card">
      ...
      <a class="btn btn-primario" href="${ruta("/checkout")}">Ir a pagar →</a>
    </div>
...
function renderCheckout(): void {
  ...
  if (resumen.vacio) {
    cont.innerHTML = `<div class="card"><p class="soft">No hay nada para pagar.
      <a href="${ruta("/")}#catalogo">Elegí una plantilla →</a></p></div>`;
    return;
  }
