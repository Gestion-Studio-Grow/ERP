...
test("todas las páginas cargan app.js y el CSS con el prefijo de montaje (BASE)", () => {
  const appJs = `<script type="module" src="${ruta("/app.js")}">`;
  const css = `href="${ruta("/globals.css")}"`;
  for (const html of [landing(), carrito(), checkout(), gracias(), producto(PLANTILLAS[0])]) {
    assert.ok(html.includes(appJs), `falta ${appJs}`);
    assert.ok(html.includes(css), `falta ${css}`);
  }
});
