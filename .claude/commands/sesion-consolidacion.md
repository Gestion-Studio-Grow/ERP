...
6. **GitHub y deploy**: la fuente de verdad es GitHub → `main` == `origin/main` (`git log origin/main..main` vacío). El auto-publish de Netlify está **apagado** (`stop_builds`, ver `docs/METODO-ROLES.md` §4): `main` **puede** estar adelante del último deploy `ready` sin que sea un bug — es lo esperado para commits de tooling/docs. Lo que se verifica: (a) `main`==`origin/main`; (b) si hay commits sin deployar que **tocan `src/` o `prisma/`**, que quede claro que el no-deploy es intencional (no un deploy olvidado). Consultar Netlify: `npx netlify api listSiteDeploys --data '{"site_id":"1865126e-8dc2-4ce8-8377-74a669e89610","per_page":2}'`.
