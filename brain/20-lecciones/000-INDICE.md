...
- **[DX-5](DX-5-no-es-la-copia-de-la-web-real-hay-que-afinar-el-lapiz-magra.md)** — en todo cliente "réplica exacta" — (a) extraer el sitio real con navegador (tree +
- **[DX-6](DX-6-una-relacion-seedeada-uniforme-hace-que-el-front-mienta-por-.md)** — (1) el provisioning/import de catálogo real
- **[DX-7](DX-7-fix-de-dato-de-prod-con-guardarrail-sin-seed-sin-deletemany-.md)** — todo fix de dato de prod (no-migración) usa script versionado con dry-run default +
...
- **[MP-10](MP-10-reconciliar-una-rama-vieja-a-main-selectivo-no-git-merge.md)** — reconciliar rama vieja → main por pathspec/delta, no `git merge`; para cada archivo tocado por ambas, comparar contra `main` y traer solo lo nuevo. Colisión de nº de ADR → renumerar el de la rama al siguiente libre + arreglar el INDEX.
- **[MP-11](MP-11-rebase-con-conflicto-en-una-tabla-de-irreversibles-c-dividir.md)** — conflicto en una tabla/lista con IDs → antes de resolver, leer qué concepto describe cada lado; si son distintos, conservar ambos y renumerar (como la colisión de ADR de MP-10); actualizar las referencias cruzadas. Nunca `checkout --ours/--theirs` sobre filas de `§C`.
- **[MP-12](MP-12-estado-actual-md-con-drift-interno-el-handoff-avanza-pero-el.md)** — en FASE 0, verificar contra git (no contra el propio doc) los 3 anclas duras — `main HEAD` (§1), estado de frentes (§7-bis) y `.claude/agents/` (§8) — y reconciliar TODAS las secciones que citen esos hechos, no solo el banner. "Gana el repo" aplica también a las contradicciones internas.
...
- **[MP-14](MP-14-gating-por-redirect-riesgo-de-loop-si-el-destino-tambien-se-.md)** — antes de enforcar gating con `redirect()`, mapear el destino para CADA rol y CADA combinación de módulos apagados; si algún destino puede estar gateado, no redirigir — usar 404/estado neutro o esconder. Nunca redirigir a la home del rol si esa home es gateable.
- **[MP-15](MP-15-deviacion-de-una-decision-de-adr-citando-una-autoridad-no-tr.md)** — si una sesión se desvía de un ADR aceptado, trae la confirmación del dueño al mismo
