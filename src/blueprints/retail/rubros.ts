...
const carniceria: RetailRubro = {
  ...
  // `carniceria.ts`, ahora consolidado acá — el alta real de magra los pisa vía flags
  // de `provision-tenant.ts`, ver docs/tenants/magra/provisioning-magra.md).
  // `whatsapp` NO tiene default acá — regla dura: nunca un número hardcodeado/falso.
  // Sin el número real del tenant, el CTA de WhatsApp lo pide just-in-time
  // (ver src/components/whatsapp-cta.tsx) en vez de caer a un placeholder.
  brandingDefaults: {
