 ...
 * de ARCA/ANSES). PURO, sin dependencias.
 *
 * Vive en el DOMINIO FISCAL DEL CORE (`src/lib/fiscal`), no en un plugin: es una
 * regla fiscal argentina que el Core necesita para validar CUIT antes de emitir un
 * comprobante, independientemente del plugin de bancos. (Antes vivía en
 * `plugins/bancos/domain/cuit.ts` y varios `-core` la importaban desde el plugin →
 * acoplamiento core→plugin prohibido por ADR-002; movida acá, el plugin ahora la
 * re-exporta desde el Core — dirección plugin→core permitida.)
 *
 * Nota de alcance: en el resto del repo (suppliers, formal-order) se valida solo
 * FORMA a propósito (datos de terceros cargados a mano); acá el dato va derecho a un
 * comprobante fiscal, por eso el verificador es obligatorio.
 */
