...
> - **Módulos del backoffice (ADR-054/055):** vidriera **`/admin/modulos`** (ABM de apps, variante + dependencias, cap `modules:manage`) **+ gating de navegación** (`src/modules/gating.ts` + `src/lib/module-gating.ts`): con `MODULE_REGISTRY_ENABLED` on, apagar un módulo lo saca del menú; default off = nav legada intacta (reversible). Primer consumidor del backoffice para la fundación de módulos. Vallas verdes (tsc + **572 tests** + build + gate:rls 38/38 cobertura estática + lint).
>   - **✏️ CORRECCIÓN 2026-09-17 — este renglón decía "el OWNER prende/apaga las apps". Es FALSO.** `modules:manage` **no es una capacidad del dueño**, y no es un olvido: está excluida a propósito y con el motivo escrito en `src/lib/capabilities.ts:71-82` (*"decisión comercial: se aprovisiona desde la consola de operador"*). La exclusión vive **en el tipo** (`Record<Exclude<Capability, keyof typeof NO_SON_DEL_DUENIO>, true>`, `capabilities.ts:84`), así que el compilador impide volver a dársela por descuido. Consecuencia práctica: el ítem "Módulos" sigue declarado en el menú —la ruta tiene que estar en el mapa para el gating por módulos— pero **no lo ve nadie** (`src/lib/admin-nav-items.ts:127-132`), y `/admin/modulos` corta con `requireCapability("modules:manage")` (`src/app/admin/(dashboard)/modulos/page.tsx:37-38`). Los módulos se aprovisionan desde **`/operador`**. Se verifica: `grep -n "modules:manage" src/lib/capabilities.ts`.
>   - **✏️ ACTUALIZACIÓN 2026-09-24 (tanda 2b) — `/admin/modulos` ya no existe.** Se borraron la página, su action (`src/lib/modulos-actions.ts`, `toggleModulo`) y el ítem "Módulos" del menú (`src/lib/admin-nav-items.ts:127-129` deja la nota): nadie podía abrirla y no hay llamadores. CH no pierde nada visible. Los módulos se asignan sólo desde la ficha del negocio en `/operador` (vista previa, candado de CH, auditoría). El trinquete de páginas sin `requireApp` bajó a 10 (`src/apps/guardia-paginas.test.ts:34`).
>   - **✏️ CORRECCIÓN 2026-09-17 — `MODULE_REGISTRY_ENABLED` NO se prende.** Es global del deploy (`src/modules/flags.ts:13-19`): con el flag ON el menú se arma sólo con `Tenant.modules[]`, y `beauty-spa` —único cliente vivo— tiene `modules = {}`, así que **quedaría sin menú**. El eje de gating vigente es el **rubro** (`Tenant.blueprintId` → `isRetail`, `src/lib/carniceria/rubro.ts:26-37`). Ruling completo en `docs/producto/diseno-tienda-modulos.md` §RULING.
...
| Ítem | Valor |
|---|---|
| **main HEAD (origin)** | **`7ccee77`** — `merge: fix login staging — seed OWNER (B1) + fiado sin doble-conteo (M1)`. Cierra el bloqueante del audit de backend (S5): **B1** — `seed-magra.ts` de main ahora **crea el usuario OWNER** `dueno@magra-demo.test` (scrypt, rol OWNER, `active`); antes main tenía el seed viejo SIN usuario → login "email o contraseña incorrectos". **M1** — el fiado se modela **una sola vez** (cuenta a cobrar; `pedido2` impago en cuenta corriente, sin `Collection` ORDER) → sin doble-conteo. Seed: baranda anti-prod dura + idempotente. **tsc + 729 tests + adr:linkcheck (59/0).** El seed es **SOLO dev/qa** (corre contra `qa-empresa`, no Neon prod). *Staging: `FORCE_TENANT_SLUG=magra-demo` (I1) + `AUTH_SECRET` (I2) + migraciones aplicadas en qa-empresa (I3) ya seteados por el dueño → falta `git pull` + re-seed.* **Riesgos abiertos del audit (no bloqueantes):** I2 fallback `dev-secret` de `AUTH_SECRET` (hacer required fuera de dev) · I3 loaders Empresa sin fallback si falta la tabla. Sobre GEP-Etapa-1 (`2cc0cb5`): **(1) UX densidad por perfil** — `layout` setea `data-density` (Comercio 1.32 espacioso / Empresa 1 denso), reusa tokens existentes; **detrás de `PROFILES_ENABLED` → flag OFF = byte-idéntico** (React omite el atributo); canal neutro intacto; helper puro `profile-density.ts` + tests. **(2) Consola MODULES canónico** — `operator-config.MODULES` **deriva** de `DESCRIPTORES_CATALOGO` (fuente única, no 2ª lista; cierra riesgo tipo FU1; +`reviews`) + **5 módulos Empresa ADR-060** (10→16); guarda anti-drift; `provisionTenant.profile` **NO tocado** (§C/A-B elevado). **tsc + 729 tests + adr:linkcheck (59/0).** Nada toca prod/§C. **GEP Etapa 1 (`2cc0cb5`):** grafo 65 nodos consistente + cargador de contexto + gobernanza. Sobre FU1+GEP-Etapa-0 (`6e8dc7a`): **GEP Etapa 1** — grafo **consistente 65 nodos, 0 edges colgadas** (cierra la observación de Etapa 0: AMD + 5 sub-enmiendas como nodos) · `scripts/adr-context.mjs` (cargador de contexto pre-sprint, FASE 0) · gobernanza `docs/metodologia/gobernanza-conocimiento-adr.md` con `adr:linkcheck` (✅ 59 frontmatter completo, 0 refs rotas), `adr:graph` (regenera en retro), `adr:context`. **100% aditivo** (cero src/, cuerpos/IDs de ADR intactos, INDEX intacto). Nada toca prod/§C. *(RFC-002 UX + RFC-003 alta-tenants + Challenger S5 también en `docs/rfc/`, doc-only.)* **FU1:** `defaultModulesForBlueprint` fuente única. **GEP Etapa 0:** Constitución C-001..C-005 + frontmatter en los 59 ADR. **↓ debajo sigue el estado ADR-060 (D1–D10) CONSTRUIDO ENTERO.** Sobre Fase D/E (`2b3da85`): **devolución a proveedor (D4)** dos patas **atómicas** (stock `DEVOLUCION_PROVEEDOR` + crédito en `AccountPayable` vía `Collection`, tx Serializable → aborta todo si sobre-crédito o falla stock) · **inventario light (D5)** = read-model de valuación **sin schema** · fix **Badge neutral** (marca CH → token `surface-sunken`; tier realmente neutro). Gating alineado: inventario+CxC **lite+rubro**, devoluciones+CxP **enterprise** (nav reconciliada por S5). Invariante `enterprise ⊇ lite`, sin dead-ends. **`gate:rls` 38/38, tsc + 720 tests.** Migraciones (enum D4 + D1/D9/D10 + D2/D3 + cola Decimal) **aditivas, escritas NO aplicadas**, con rollback.sql. **Flags DEFAULT OFF.** §C sin ejecutar: **nada aplicado a Neon**, sin deploy. **Próximo: QA en preview** (`docs/runbooks/qa-preview-empresa-2026-07-08.md`). |
...
| **I7** | **Material real de Shine y A Dos Manos** — ✅ **PARCIAL: el dueño entregó el catálogo oficial de Shine (PDF Canva) el 2026-09-02**: 42 productos+precios+fotos reales ya integrados a la demo (`docs/diseno/catalogo-shine-2026.md`). **Queda abierto:** testimonios reales de IG/WhatsApp de Shine + todo el material de A Dos Manos | acción dueño (testimonios + material ADM) | el catálogo cerró la mitad Shine del DX-5; faltan reseñas reales y ADM |
