...
## 12-bis. Estado de implementación (avance con simulador, 2026-07-04)

En `main`, todo con stubs (cero DB/red/credenciales), verificado con `tsc` + smokes:
- **OAuth (§9):** `src/plugins/mercadopago/oauth.ts` — `MercadoPagoOAuthPort` + `StubMercadoPagoOAuth` (URL de autorización, `intercambiarCodigo`, `refrescar`), `CredencialesStore` + stub en memoria, `vincularCuenta` (el comercio autoriza → se guardan tokens **por tenant**) y `CredencialesConRefresh` (renueva el access token al vencer). Listo para enchufar credenciales reales.
- **Clasificación → facturación end-to-end (§12.1):** `src/lib/mercadopago-simulador.ts` — `simularIngestaCliente` cierra el pipeline: feed MP → clasifica → factura las ventas contra el **stub de ARCA** (CAE simulado), descarta transferencias/devoluciones. Idempotente.
- **Panel del contador (§12.2):** `src/app/contador/` (ruta separada del `/admin` del tenant, plano cross-tenant) + `src/lib/contador-panel.ts` — cartera multi-cliente con datos vivos del simulador (cobrado por MP, facturado/no/revisar, % del tope de recategorización con alerta ≥80%). Scaffold con datos simulados.

Pendientes de la lista de §8 (DB real, OAuth real, worker, etc.) siguen diferidos.

**Avance nocturno (2026-07-04, mismo criterio con stubs):**
- **Clasificador v2 (§12.1):** config por comercio (`cuentasPropias` → transferencias a cuentas propias = NO_FACTURABLE) + reglas extra; **aprendizaje** (`AprendizajePort`): las correcciones del comercio/contador sobre un REVISAR se recuerdan (por contraparte / operación) y se reaplican solas.
- **Robustez (§8/§4):** `ErrorFacturacion` retriable/terminal → rechazo de ARCA = `RECHAZADO` (no reintenta); error transitorio reintenta hasta un tope y escala a `REVISAR`. Conciliación con estados FACTURADO/NO_FACTURABLE/REVISAR/RECHAZADO/ERROR + `intentos`; `yaProcesado` = idempotencia sobre terminales.
- **Webhook + backfill unificados (§2):** `src/lib/mercadopago-ingest.ts` — `procesarWebhookPago` y `sincronizarHistorico` comparten conciliación + aprendizaje por tenant en el **mismo** camino idempotente; ver un pago por las dos fuentes **no duplica** la factura.
- **Panel del contador enriquecido (§12.2/§12.3):** desglose de conciliación por cliente, **alerta de recategorización** (≥80% del tope) y sección de pendientes con acción en lote (scaffold).
