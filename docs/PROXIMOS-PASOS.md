...
- [ ] `/sesion-arquitectura Plugin ARCA: contrato de eventos/comandos + build-vs-buy del conector` — retoma el "Diseño detallado del Plugin ARCA" del INDEX y le suma la decisión relevada del mercado: definir el contrato concreto (`InvoiceCreated` → `RegisterFiscalDocument`, manejo de CAE/vencimiento/errores de ARCA, idempotencia por AMD) **y** elegir el conector inicial (comprar TusFacturas API / AfipSDK vs construir WS propios WSAA/WSFE), como implementación reemplazable detrás del plugin. Recomendación de negocio: **Buy primero, Build a escala** + alcance mínimo B/C monotributo/servicios. **Sirve a los dos productos** (facturador horizontal y facturador para estudios contables); para el segundo es **multi-CUIT** (certificado por tenant-cliente) — anotar esa dimensión. Insumos: `docs/facturador-electronico-arca-mercado-y-vision.md` y `docs/facturador-estudios-contables-mercado-y-vision.md`. *(origen: sesión de negocio — 2026-07-04)*
