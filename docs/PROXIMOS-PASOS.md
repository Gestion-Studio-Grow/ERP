...
- [ ] `/sesion-consolidacion` (o feature chico) — **higiene de schema, bajo, no urgente:** columnas huérfanas detectadas en la consolidación 2026-07-04 que no se tocaron por requerir migración sobre la base de producción: `Client.birthDate` (nunca leída/escrita; podría alimentar "ficha de cliente más rica"), `Appointment.isResidentBooking` y `User.lastLoginAt` (write-only, se escriben pero nadie las lee). `Tenant.timezone` queda fuera: su cableado por-tenant es parte del trabajo de RLS/multi-tenant (ADR-018), no hygiene suelta. *(origen: consolidación — 2026-07-04)*
...
_(sin ítems — la consolidación 2026-07-04 barrió "RBAC Fase 1", verificado contra el código: `User`+`UserRole`, login por email/password con scrypt, `getCurrentUser()` y actor real en el audit. Su continuación, RBAC Fase 2, sigue en Abiertos.)_
