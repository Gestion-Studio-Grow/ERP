...
- [ ] `/sesion-arquitectura o /sesion-feature Plugin ARCA (contrato concreto)` — **disparo: cuando se encare la facturación fiscal real (Fase 2).** Instancia las superficies II/III de ADR-020: comando público `RegisterFiscalDocument(invoiceId, cae, vencimientoCae)` (Plugin→Core), evento `InvoiceCreated` (Core→Plugin) y el **outbox** de ADR-002 (recién acá se construye, no antes). Materializa la capability **Factura**, hoy descrita pero vacía (ADR-020 §6.a). Declara el manifiesto del Plugin según ADR-006. *(origen: sesión de arquitectura — 2026-07-04)*
- [ ] `/sesion-feature plegar cruce Scheduling→Stock a comando público (ADR-020 §5.a)` — **disparo: cuando se construya la capability Stock de verdad (Fase 2, venta de productos físicos) o aparezca un 2º consumidor de "descontar stock".** Hoy `completeAppointment` escribe `Product.stock` directo dentro de su transacción (grandfathered a propósito: en un monolito de un proceso la transacción ya da atomicidad, reescribir sin 2º consumidor es refactor por el refactor). Extraer a un comando `Producto.consumeStock(...)` cuando haya segunda razón. **No** hacerlo antes. *(origen: sesión de arquitectura — 2026-07-04)*
- [ ] `materializar capability Orden (ADR-020 §6.a) — solo si` aparece una venta multi-línea que el 1:1 `Appointment↔Payment` no cubra (varios servicios/productos en un comprobante). Hoy no hace falta: Pago cuelga directo del Turno. Reservada, no construir especulativamente. *(origen: sesión de arquitectura — 2026-07-04)*
- [ ] `/sesion-feature consola de operación / super-admin (ADR-021)` — **disparo: cuando se provisione el 2º tenant; es el mismo trabajo que ADR-018 + ADR-019 (Fase 1).** Superficie **separada** de operador con credencial propia por entorno (no reusar la sesión del tenant; **no** agregar `SUPERADMIN` a `UserRole`) y **conexión/rol de DB propios** (único camino cross-tenant / `BYPASSRLS`, aislado del Prisma Client del tenant — si no, se derrota RLS desde adentro). Contenido mínimo: listar tenants + salud (read-heavy), ejecutar el provisioning envolviendo el script de ADR-019, ver/operar el gate de RLS. Impersonation de soporte, baja de tenant, planes/flags (ADR-006) y UI rica quedan en Fase 2 (diferida). **Fase 0 (hoy): NO construir por defecto** — con 1 tenant los dashboards de Neon/Netlify ya cubren la salud; solo si hay fricción operativa medible vale una vista read-only mínima. *(origen: sesión de arquitectura — 2026-07-04)*
