...
- [ ] **Aplicar la migración `20260704120000_add_business_settings` a Neon** (`npx prisma migrate deploy`) y hacer el deploy del módulo Localización — **bloqueado en la sesión que lo implementó porque el `.env` local tenía `DATABASE_URL` vacío.** La migración es aditiva (crea solo la tabla `BusinessSettings`, sin tocar datos existentes) y sin ella `/admin/localizacion` y el lector `getLocation` fallan en runtime aunque el build compile. Requiere la connection string real de Neon + push a `main` (deploy). *(origen: sesión de feature — 2026-07-04)*
- [x] `/sesion-feature Módulo Localización` — hecho, verificado (tsc + build en verde), **sin deployar** (ver ítem abierto de arriba: falta aplicar migración + push). Ficha de ubicación/contacto editable por el OWNER (`/admin/localizacion`, capacidad `location:manage`), modelo `BusinessSettings` singleton por tenant con defaults (`LOCATION_DEFAULTS`) para que la web nunca quede rota, lector público cacheado `getLocation` (`src/lib/settings.ts`) que alimenta hero + footer + "Dónde estamos", Server Action con upsert + audit (`settings-actions.ts`). Reemplaza los datos que estaban hardcodeados en el sitio y en `business-config.ts`. Fuera de alcance a propósito: el título/description SEO de `src/app/layout.tsx` y el campo `LOCATION` del `.ics` en `BookingModal` siguen con la dirección fija (son estáticos/generados aparte); wirearlos a `BusinessSettings` queda como mejora futura. Commiteado en rama `feature/modulo-localizacion` (no en `main`). *(origen: sesión de feature — 2026-07-04)*
