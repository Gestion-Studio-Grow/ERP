...
- [ ] `/sesion-feature Plugin ARCA — emisión con CAE (ADR-019)` — implementar lo decidido en ADR-019: modelos `FiscalDocument` + `TenantFiscalConfig` + `Outbox` (todos con `tenantId`) y migración en Neon; creación de `FiscalDocument`+outbox dentro de la transacción de venta; interfaz `FiscalConnector` + adaptador **AfipSDK** (alcance mínimo `FACTURA_B`/`FACTURA_C`); comandos idempotentes `RegisterFiscalAuthorization`/`RegisterFiscalRejection`; **Netlify Scheduled Function** que drena la outbox con reintentos (B3: síncrono best-effort + outbox durable como red); config fiscal por tenant con credencial fuera del repo. **Ensayo obligatorio contra el entorno de homologación de ARCA antes de tocar CUIT productivo.** Cierra la Fase 2 del piloto. **Nace con la forma extensible de ADR-020** (aunque implemente solo Fase 1): `FiscalConnector` con capacidades declaradas, `TenantFiscalConfig` + `TenantJurisdiccion` como relación, namespace `ar.nacional` explícito — costo marginal ~cero ahora, caro retrofitear. Insumos: `docs/adr/ADR-019-plugin-arca-contrato-y-conector.md` + `docs/adr/ADR-020-localizacion-argentina-subsistema-extensible.md`. *(origen: sesión de arquitectura — 2026-07-04)*
