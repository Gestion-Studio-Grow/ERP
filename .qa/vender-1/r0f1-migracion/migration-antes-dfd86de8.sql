...
-- la política `tenant_isolation` (idéntica a la de prisma/rls/0001_enable_rls.sql). Una base
-- creada sólo con migraciones ya no deja estas tablas sin aislar. Re-ejecutar 0001 después es
-- inocuo (borra y vuelve a crear la misma política). Los GRANT al rol de la app los da el
...
--
-- LLAVES FORÁNEAS CON EL NEGOCIO ADENTRO: cuando una tabla cuelga de otra de negocio, la llave
-- es (tenantId, x) → padre (tenantId, id). La base impide que una fila de un negocio apunte a la
-- de otro aunque el código se equivoque de id (las verificaciones de llaves foráneas no pasan por
-- RLS). Contactos, conversaciones, extractos y delegaciones cuelgan de CarteraCliente: no se
-- anota nada de un negocio que no está en la cartera del estudio.
--
...
-- CreateIndex
CREATE INDEX "IntegracionCredencial_tenantId_conexionId_idx" ON "IntegracionCredencial"("tenantId", "conexionId");

-- CreateIndex
CREATE UNIQUE INDEX "IntegracionCredencial_conexionId_campo_key" ON "IntegracionCredencial"("conexionId", "campo");
...
-- CreateIndex
CREATE INDEX "EventoIntegracion_tenantId_conexionId_idx" ON "EventoIntegracion"("tenantId", "conexionId");

-- CreateIndex
CREATE UNIQUE INDEX "EventoIntegracion_conexionId_direccion_idExterno_key" ON "EventoIntegracion"("conexionId", "direccion", "idExterno");

-- CreateIndex
CREATE INDEX "IntegracionUso_tenantId_idx" ON "IntegracionUso"("tenantId");

-- CreateIndex
CREATE UNIQUE INDEX "IntegracionUso_conexionId_mes_key" ON "IntegracionUso"("conexionId", "mes");
